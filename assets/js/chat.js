const text_box = '<div class="card-panel right" id = "ff"  style="width: 75%; position: relative">' +
    '<div style="position: absolute; top: 0; left:3px; font-weight: bolder" class="title">{sender}</div>' +
    '<div class = "fsdg">{message}</div>'+ 
    '</div>';
            ...
            if (data.length !== 0) {
                for (var i = 0; i < data.length; i++) {
                    console.log(data[i]);
                        ...
                        box = box.replace('ff', data[i].suma);
                        $('#board').append(box);
                        javascript:console.log(box);
